...
/**
 * 这是一个对listview直接继承封装的上拉刷新，下拉加载的listview，功能可用代码简单
 * 在控件层级上可以继续优化一下，有时间修补一下
 * 使用方法：
 * setPullLoadEnable(true);//开启加载更多
   setPullRefreshEnable(true);//开启下拉刷新
   setXListViewListener(IXListViewListener);
   IXListViewListener中实现
   onRefresh()//下拉刷新的时候会触发该方法
   setRefreshTime(String)//设置要显示的刷新时间
   stopRefresh();//让下拉刷新的圈圈消失
   
   onLoadMore()//上拉加载更多时触发,历史数据
   stopLoadMore();//让加载更多的圈圈消失
